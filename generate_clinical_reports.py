...
def _add_user_site_packages():
    ...
    # Try standard site.getusersitepackages()
    try:
...
def _find_template():
    """Locate template.docx — prefer repo root, fall back to ./0325/."""
...
def generate_report(case_id, tiered_csv, annotated_csv, output_path):
    ...
    for v in tier12:
        ...
        gene = v["Gene"]
        # KB overrides the hardcoded THERAPEUTIC dict so the report picks
        # up phrasing learned from past signed-out reports. Hardcoded
        # entries still act as the fallback.
        therapeutic = (
            _KB_THERAPEUTICS.get(gene)
            or THERAPEUTIC.get(gene, "")
        )
        values = [
    ...
    for p in cell.paragraphs:
        ...
        for child in list(p_el):
            if child.tag.endswith('}r'):
                p_el.remove(child)
    # Try exact match first, then prefix match for INTERPRETATIONS
    interp = INTERPRETATIONS.get(case_id, "")
    if not interp:
        # Prefix match: case_id "01-JJH_10679562" should match key "01-"
        for key, text in INTERPRETATIONS.items():
            if case_id.startswith(key):
                interp = text
                break
    if not interp and _KB_INTERPRETATIONS:
        # Fall back to the closest KB interpretation keyed by case_id prefix.
        # KB keys look like "05-|gene1+gene2" — match on the case_id part.
        for k, text in _KB_INTERPRETATIONS.items():
            kb_case = k.split("|", 1)[0]
            if case_id == kb_case or case_id.startswith(kb_case):
                interp = text
                break
    p = cell.paragraphs[0]
